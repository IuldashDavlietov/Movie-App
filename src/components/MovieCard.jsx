...
export const MovieCard = ({ movie }) => {
  ...
  const { movie: details, loading } = useMovieDetails(
    ...
    <Link to={`/details/${movie.id}`} className="block">
      ...
      >
        <div className="relative aspect-2/3 w-full bg-gray-900 overflow-hidden rounded-xl">
          ...
          >
            <h4 className="text-base font-bold text-white mb-1 drop-shadow-[0_2px_2px_rgba(0,0,0,0.9)]">
              {title}
            </h4>
            <p className="text-xs text-gray-200 mb-4 drop-shadow-[0_1px_1px_rgba(0,0,0,0.9)]">
              {releaseYear}
            </p>

            {loading ? (
              <div className="text-xs text-cyan-400 animate-pulse font-medium">
                Download...
              </div>
            ) : (
